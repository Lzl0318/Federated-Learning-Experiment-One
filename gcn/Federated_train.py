...
def plot():
    plt.figure()
    ax1 = plt.subplot(221)
    plt.plot(np.linspace(0, len(loss_train_list[3]), len(loss_train_list[3])), loss_train_list[3], label='bob federated', color='blue')
    plt.plot(np.linspace(0, len(loss_train_list[4]), len(loss_train_list[4])), loss_train_list[4], label='alice federated', color='red')
    plt.plot(np.linspace(0, len(loss_train_list[0]), len(loss_train_list[0])), loss_train_list[0], label='bob alone', color='yellow')
    plt.plot(np.linspace(0, len(loss_train_list[1]), len(loss_train_list[1])), loss_train_list[1], label='alice alone', color='green')
    plt.plot(np.linspace(0, len(loss_train_list[2]), len(loss_train_list[2])), loss_train_list[2], label='local train', color='black')
    plt.legend(ncol=2)
    plt.xlabel('epoch')
    plt.ylabel('loss')
    plt.title('train loss')
    ax2 = plt.subplot(222)
    plt.plot(np.linspace(0, len(accuracy_train_list[3]), len(accuracy_train_list[3])), accuracy_train_list[3], label='bob federated', color='blue')
    plt.plot(np.linspace(0, len(accuracy_train_list[4]), len(accuracy_train_list[4])), accuracy_train_list[4], label='alice federated', color='red')
    plt.plot(np.linspace(0, len(accuracy_train_list[0]), len(accuracy_train_list[0])), accuracy_train_list[0], label='bob alone', color='yellow')
    plt.plot(np.linspace(0, len(accuracy_train_list[1]), len(accuracy_train_list[1])), accuracy_train_list[1], label='alice alone', color='green')
    plt.plot(np.linspace(0, len(accuracy_train_list[2]), len(accuracy_train_list[2])), accuracy_train_list[2], label='local train', color='black')
    plt.legend(ncol=2)
    plt.xlabel('epoch')
    plt.ylabel('accuracy')
    plt.title('train accuracy')
    ax3 = plt.subplot(223)
    plt.plot(np.linspace(0, len(loss_val_list[3]), len(loss_val_list[3])), loss_val_list[3], label='federated', color='blue')
    plt.plot(np.linspace(0, len(loss_val_list[0]), len(loss_val_list[0])), loss_val_list[0], label='bob alone', color='red')
    plt.plot(np.linspace(0, len(loss_val_list[1]), len(loss_val_list[1])), loss_val_list[1], label='alice alone', color='green')
    plt.plot(np.linspace(0, len(loss_val_list[2]), len(loss_val_list[2])), loss_val_list[2], label='local train', color='black')
    plt.legend(ncol=2)
    plt.xlabel('epoch')
    plt.ylabel('loss')
    plt.title('val loss')
    ax4 = plt.subplot(224)
    plt.plot(np.linspace(0, len(accuracy_val_list[3]), len(accuracy_val_list[3])), accuracy_val_list[3], label='federated', color='blue')
    plt.plot(np.linspace(0, len(accuracy_val_list[0]), len(accuracy_val_list[0])), accuracy_val_list[0], label='bob alone', color='red')
    plt.plot(np.linspace(0, len(accuracy_val_list[1]), len(accuracy_val_list[1])), accuracy_val_list[1], label='alice alone', color='green')
    plt.plot(np.linspace(0, len(accuracy_val_list[2]), len(accuracy_val_list[2])), accuracy_val_list[2], label='local train', color='black')
    plt.legend(ncol=2)
    plt.xlabel('epoch')
    plt.ylabel('accuracy')
    plt.title('val accuracy')
    plt.show()
    # plt.savefig(r'C:\Users\lzl_z\Desktop\Fed_GCN_Experiment_one\image\E'+str(args.E)+'_'+str(args.zubie)+'.png')
...
def final_test(ls):
    ...
    with open(r'C:\Users\lzl_z\Desktop\Fed_GCN_Experiment_one\citeseer-results\testing\E' + str(args.E) + '_' + str(
...
with open(r'C:\Users\lzl_z\Desktop\Fed_GCN_Experiment_one\citeseer-results\validation\E'+str(args.E)+'_'+str(args.zubie)+'.pkl', 'wb') as f:
    pkl.dump(val_info_dict, f)
f.close()

# model saving
# with open(r'C:\Users\lzl_z\Desktop\Fed_GCN_Experiment_one\model\bob_alone_E'+str(args.E)+'_'+str(args.zubie)+'.pkl', 'wb') as f1:
#     torch.save(bob_alone_model, f1)
# f1.close()
# with open(r'C:\Users\lzl_z\Desktop\Fed_GCN_Experiment_one\model\alice_alone_E'+str(args.E)+'_'+str(args.zubie)+'.pkl', 'wb') as f2:
#     torch.save(alice_alone_model, f2)
# f2.close()
# with open(r'C:\Users\lzl_z\Desktop\Fed_GCN_Experiment_one\model\local_E'+str(args.E)+'_'+str(args.zubie)+'.pkl', 'wb') as f3:
#     torch.save(local_model, f3)
# f3.close()
# with open(r'C:\Users\lzl_z\Desktop\Fed_GCN_Experiment_one\model\fed_E'+str(args.E)+'_'+str(args.zubie)+'.pkl', 'wb') as f4:
